 ...
 */
package org.apache.rocketmq.connect.metrics.utils;
    ...
    SIMPLE("^([\\+\\-]?\\d+)([a-zA-Z]{0,2})$") {
        ...
        public Duration parse(String value, ChronoUnit unit) {
            try {
                ...
                if (!matcher.matches()) {
                    throw new IllegalArgumentException("Does not match simple duration pattern");
                }
                String suffix = matcher.group(2);
                return (StringUtils.isNotBlank(suffix) ? Unit.fromSuffix(suffix)
