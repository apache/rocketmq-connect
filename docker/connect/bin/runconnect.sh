...
if [[ $(check_java_version "$JAVA" "9") == "false" ]]; then
  JAVA_OPT="${JAVA_OPT} -Xloggc:/dev/shm/mq_gc_%p.log -XX:+PrintGCDetails -XX:+PrintGCDateStamps -XX:+PrintGCApplicati
  toppedTime -XX:+PrintAdaptiveSizePolicy"
